...
pub fn present() -> bool {
    let mut cpuid = __cpuid(CPUID_PROCESSOR_INFO_AND_FEATURE_BITS);
    if (cpuid.ecx & CPUID_FEATURE_HYPERVISOR) == 0 {
        return false;
    }

    cpuid = __cpuid(CPUID_GET_HIGHEST_FUNCTION);
    if cpuid.eax < CPUID_HYPERV_VENDOR_AND_MAX_FUNCTIONS {
        return false;
    }

    cpuid = __cpuid(CPUID_HYPERV_VENDOR_AND_MAX_FUNCTIONS);
    ...
    cpuid = __cpuid(CPUID_HYPERV_FEATURES);
    ...
    cpuid = __cpuid(CPUID_HYPERV_ISOLATION_CONFIG);
