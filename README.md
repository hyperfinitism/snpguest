...
| `generate` | `measurement`, `ovmf-hash`, `id-block` | Calculates reference values |
...
The user must provide the path to an attestation report `$ATT_REPORT_PATH` to get the reported TCB and chip ID. If the V3+ report is given, the command automatically detects the host processor model. Otherwise, the user must also provide the host processor model (`milan`, `genoa`, `bergano`, `sienna`, `turin`) via the `-p, --processor-model` option.
